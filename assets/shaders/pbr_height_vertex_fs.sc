...
vec4 compute_terrain_color(vec3 pos, vec3 normal)
{
	
	vec2 mf = mineral_fading * terrain_scale.w;
	vec4 color_mineral = mix(color_mineral1, color_mineral2, smoothstep(mf.x, mf.y, pos.y));

	vec4 color_terrain = mix(color_mineral, color_grass, smoothstep(normal_params.y, normal_params.z, normal.y));
	
	
	if (pos.y < water_altitude.x * terrain_scale.w)
	{
...
void main() {
	...
	vec3 normal_terrain = vNormal;
	vec4 base_opacity = compute_terrain_color(vWorldPos, normal_terrain);
	//
