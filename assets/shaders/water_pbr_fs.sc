...
uniform vec4 transparency_fresnel; //x=fresnel near (1.0 is nearest), y= fresnel far (0.0 is farest)
uniform vec4 waves_scale;
uniform vec4 plane_scale;

// Texture slots
SAMPLER2D(water_noises, 0);
...
{
	float time = uClock.r * waves_scale.w;
	vec2 p = pa * waves_scale.xz;
	vec2 v0 = texture2D(water_noises, p).rg;
	float a = texture2D(water_noises, v0 + vec2(0.025, -0.033) * time ).b;
	return a;
}
...
float get_wave_altitude(vec2 p)
{
	...
	return a * waves_scale.y;
}


vec3 get_normale_plane(vec2 p_surface)
{
	float f = 0.01; //0.02 for rt noise
	vec2 xd = vec2(f, 0.0);
	vec2 zd = vec2(0.0, f);
...
void main() {
	...
	//vec3 normal_water = get_normale_plane(vWorldPos.xz / plane_scale.x);
	vec3 normal_water = get_normale_plane(vTexCoord0.xy);
	vec3 N = normalize(normal_water); // geometry normal
...
# endif
	...
	float fresnel_water = abs(dot(N, V));
	opacity = 1.0 - linearstep(transparency_fresnel.y, transparency_fresnel.x, fresnel_water);
